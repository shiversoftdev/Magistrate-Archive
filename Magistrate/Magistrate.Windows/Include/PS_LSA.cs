using System;
//https://github.com/ChrisAWalker/cSecurityOptions/blob/master/DSCResources/UserRightsAssignment/UserRightsAssignment.psm1
//stolen from
namespace PS_LSA
{
    ...
    internal sealed class Sid : IDisposable
    {
        ...
        public Sid(SecurityIdentifier existing)
        {
            sid = existing;
            Byte[] buffer = new Byte[sid.BinaryLength];
            sid.GetBinaryForm(buffer, 0);
    ...
    internal sealed class LsaWrapper : IDisposable
    {
        ...
        public Rights[] EnumerateAccountPrivileges(string account)
        {
            using (Sid sid = new Sid(account))
            {
                return EnumerateAccountPrivileges(sid);
            }
        }

        public Rights[] EnumerateAccountPrivileges(Sid sid)
        {
            uint ret = 0;
            ulong count = 0;
            ...
            if (ret == STATUS_OBJECT_NAME_NOT_FOUND) return Array.Empty<Rights>();  // No privileges assigned
            if (ret == STATUS_ACCESS_DENIED) throw new UnauthorizedAccessException();
            if ((ret == STATUS_INSUFFICIENT_RESOURCES) || (ret == STATUS_NO_MEMORY)) throw new OutOfMemoryException();
            throw new Win32Exception(Win32Sec.LsaNtStatusToWinError((int)ret));
        }
